...
const TICKET_CSS = `
  ...
  .nt-cta:hover { transform: translateY(-2px); box-shadow: 0 16px 32px rgba(205,72,42,.36); background: #A63A22; }
  ...
  .nt-back:hover { background: #EFEAE1; }
  ...
  .nt-ticket { width: 100%; max-width: 1060px; display: flex; border-radius: 26px; overflow: hidden; background: #FFFFFF; box-shadow: 0 30px 64px -28px rgba(60,42,28,.34); border: 1px solid #EFEAE1; }
...
const BookingOverview = () => {
  ...
  const cardStyle = (active) => ({
    ...
    border: active ? `1.5px solid ${ACCENT}` : "1px solid #E1DACE",
        ...
        <main className="nt-main">
          <div className="nt-ticket">
            ...
            <div className="nt-left">
              ...
              <div style={{ position: "relative" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
                  ...
                  >
                    <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#2E6E4E" }} />
                    Ready to book
                  </span>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px 18px" }}>
                  {details.map((d, i) => (
                    <div key={i} style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      ...
                      <span style={{ font: `600 16px/1.2 ${BODY_FONT}`, color: "#F7F4EF" }}>{d.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* stickman travel companion */}
              <svg className="sm-stage" viewBox="0 0 360 150" preserveAspectRatio="xMidYMax meet" fill="none" stroke="#F7F4EF" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                  ...
                  <g className="sm-bob">
                    <g transform="translate(60,118)">
                      ...
                      <g className="sm-breathe">
                        ...
                        <circle cx="-9" cy="-44" r="1.4" fill="#F7F4EF" stroke="none" />
                        ...
                        <g className="sm-head">
                          ...
                          <g className="sm-face-side">
                            <circle cx="3.6" cy="-72" r="0.95" fill="#F7F4EF" stroke="none" />
                            <circle cx="6.8" cy="-72" r="0.95" fill="#F7F4EF" stroke="none" />
                            <path d="M3,-66.4 Q6,-63.8 9,-66.4" vectorEffect="non-scaling-stroke" />
                          </g>
                          <g className="sm-face-front">
                            <circle cx="-3.2" cy="-72" r="0.95" fill="#F7F4EF" stroke="none" />
                            <circle cx="3.2" cy="-72" r="0.95" fill="#F7F4EF" stroke="none" />
            ...
            <div className="nt-right">
              <div style={{ font: `700 12px/1 ${BODY_FONT}`, letterSpacing: ".14em", textTransform: "uppercase", color: "#A79D8C", marginBottom: "6px" }}>
                Payment summary
              </div>
              ...
              {lineItems.map((li, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", padding: "14px 0", borderBottom: "1px solid #EFE7DA" }}>
                  <span style={{ flex: 1, font: `600 15px/1.3 ${BODY_FONT}`, color: "#33302A" }}>{li.label}</span>
                  <span style={{ font: `400 14px/1.3 ${BODY_FONT}`, color: "#A79D8C", whiteSpace: "nowrap" }}>{li.qty}</span>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", marginTop: "16px", padding: "18px 20px", background: "linear-gradient(135deg,#F3F3F3,#EDEDED)", border: "1px solid #E2E2E2", borderRadius: "14px" }}>
                <span style={{ fontFamily: DISPLAY_FONT, fontWeight: 700, fontSize: "clamp(17px,2vw,20px)", color: "#33302A" }}>Total Trip Amount</span>
                <span style={{ fontFamily: BODY_FONT, fontWeight: 800, fontSize: "clamp(19px,2.4vw,24px)", color: "#1E1C18" }}>₹ {inr(Total)}</span>
              </div>

              {partialAvailable && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center", justifyContent: "space-between", marginTop: "14px", padding: "15px 18px", background: "#EFEAE1", border: "1px dashed #E0CFBE", borderRadius: "14px" }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ font: `600 15px/1.3 ${BODY_FONT}`, color: "#33302A" }}>Book now, pay the rest later</div>
                    <div style={{ marginTop: "5px", font: `400 13px/1.45 ${BODY_FONT}`, color: "#857C6C" }}>
                      ₹ {inr(balance)} due by <strong style={{ color: "#4E483D" }}>{balanceBy}</strong>
                    </div>
                  </div>
                  <div style={{ fontFamily: BODY_FONT, fontWeight: 800, fontSize: "clamp(18px,2vw,22px)", color: "#2E6E4E", whiteSpace: "nowrap" }}>₹ {inr(firstPay)}</div>
                </div>
              )}

              <div style={{ font: `700 12px/1 ${BODY_FONT}`, letterSpacing: ".14em", textTransform: "uppercase", color: "#A79D8C", margin: "22px 0 12px" }}>
                Select amount
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: partialAvailable ? "1fr 1fr" : "1fr", gap: "12px" }}>
                <button type="button" onClick={pickFull} className="nt-amt" style={cardStyle(!partial)}>
                  <span style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ font: `600 12px/1 ${BODY_FONT}`, letterSpacing: ".04em", textTransform: "uppercase", color: "#857C6C" }}>Full payment</span>
                    <span style={dotStyle(!partial)}><span style={{ width: 8, height: 8, borderRadius: "50%", background: !partial ? "#fff" : "transparent" }} /></span>
                  </span>
                  <span style={{ fontFamily: BODY_FONT, fontWeight: 800, fontSize: "21px", color: "#1E1C18" }}>₹ {inr(Total)}</span>
                  <span style={{ font: `400 12px/1.4 ${BODY_FONT}`, color: "#857C6C" }}>Pay the whole amount today</span>
                </button>

                {partialAvailable && (
                  <button type="button" onClick={pickPartial} className="nt-amt" style={cardStyle(partial)}>
                    <span style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ font: `600 12px/1 ${BODY_FONT}`, letterSpacing: ".04em", textTransform: "uppercase", color: "#857C6C" }}>Partial</span>
                      <span style={dotStyle(partial)}><span style={{ width: 8, height: 8, borderRadius: "50%", background: partial ? "#fff" : "transparent" }} /></span>
                    </span>
                    <span style={{ fontFamily: BODY_FONT, fontWeight: 800, fontSize: "21px", color: "#1E1C18" }}>₹ {inr(firstPay)}</span>
                    <span style={{ font: `400 12px/1.4 ${BODY_FONT}`, color: "#857C6C" }}>₹ {inr(balance)} payable later</span>
                  </button>
                )}
              </div>
              ...
              <p style={{ margin: "12px 0 0", textAlign: "center", font: `400 12px/1.4 ${BODY_FONT}`, color: "#857C6C" }}>
                🔒 You&apos;ll be redirected to the secure payment gateway
              </p>
              <p style={{ margin: "8px 0 0", textAlign: "center", font: `400 12px/1.5 ${BODY_FONT}`, color: "#857C6C" }}>
