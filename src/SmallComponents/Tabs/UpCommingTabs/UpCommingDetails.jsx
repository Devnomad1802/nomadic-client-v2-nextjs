      ...
      <Container maxWidth="xl" sx={{ px: { xs: 0, md: 0 } }}>
        <Box sx={{ width: "100%", height: { xs: "240px", md: "360px" }, borderRadius: "20px", mb: 4, mt: { xs: 2, lg: 0 }, position: "relative", overflow: "hidden" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: "absolute", top: "20px", width: "100%", px: { xs: 1, md: 2 }, zIndex: 2 }}>
            <IconButton onClick={() => navigate(-1)} sx={{ background: "#FAFAFA", "&:hover": { background: "#e1e2e3" } }}>
              <KeyboardBackspaceIcon sx={{ color: "#3F3F46" }} />
            </IconButton>
            <Box sx={{ display: "flex", gap: 1 }}>
              <IconButton sx={{ background: "#FAFAFA", "&:hover": { background: "#e1e2e3" } }}>
                <ShareIcon sx={{ color: "#3F3F46" }} />
              </IconButton>
              <IconButton sx={{ background: "#FAFAFA", "&:hover": { background: "#e1e2e3" } }}>
                <FavoriteBorderIcon sx={{ color: "#3F3F46" }} />
              </IconButton>
            </Box>
          </Box>
          <img
      ...
      <Container maxWidth="xl">
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={7.5}>
            ...
            <Typography sx={{ color: "#27272A", fontWeight: 600, fontSize: { xs: "22px", sm: "28px", lg: "33px" }, textAlign: "left", fontFamily: "Inter" }}>
              {item.title}
            </Typography>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 1, flexWrap: "wrap" }}>
              <Typography sx={{ color: "#52525B", fontSize: { xs: "13px", sm: "15px" }, textAlign: "left", fontFamily: "Inter" }}>
                {item.subTitle}
              </Typography>
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mt: 2 }}>
              {infoPills.map((pill, i) => (
                <Box key={i} sx={{ background: "#FAFAFA", border: "1px solid #F4F4F5", borderRadius: "32px", display: "flex", alignItems: "center", gap: 1, pr: 1.5 }}>
                  <IconButton size="small" sx={{ background: "#7DCD85", color: "#fff", "&:hover": { color: "#7DCD85" } }}>
                    {pill.icon}
                  </IconButton>
                  <Typography sx={{ color: "#18181B", fontSize: { xs: "13px", sm: "15px" }, fontFamily: "Inter" }}>{pill.text}</Typography>
                </Box>
              ))}
            </Box>
            ...
            <Box sx={{ display: "flex", justifyContent: "center", mt: 2 }}>
              <Box sx={{ background: "#FAFAFA", border: "1px solid #F4F4F5", borderRadius: "40px", display: "flex", alignItems: "center", gap: { xs: 1.5, md: 2.5 }, py: 1.5, px: { xs: 3, md: 5 }, width: { xs: "100%", sm: "auto" }, justifyContent: "center" }}>
                <Typography sx={{ color: "#52525B", fontSize: { xs: "14px", md: "16px" }, fontFamily: "Inter" }}>Pick Up:</Typography>
                <Typography sx={{ color: "#18181B", fontSize: { xs: "14px", md: "16px" }, fontWeight: 600, fontFamily: "Inter" }}>{item.pickUp}</Typography>
                <Box sx={{ width: { xs: "40px", md: "60px" }, display: "flex", alignItems: "center" }}>
                  <img src={line} alt="" style={{ width: "100%" }} />
                </Box>
                <Typography sx={{ color: "#52525B", fontSize: { xs: "14px", md: "16px" }, fontFamily: "Inter" }}>Drop Off:</Typography>
                <Typography sx={{ color: "#18181B", fontSize: { xs: "14px", md: "16px" }, fontWeight: 600, fontFamily: "Inter" }}>{item.dropOff}</Typography>
              </Box>
            </Box>
      ...
      >
        <Box>
          <Box sx={{ display: "flex", alignItems: "baseline", gap: 0.5 }}>
            <Typography sx={{ fontSize: "20px", fontWeight: 700, color: "#27272A", fontFamily: "Inter" }}>
              &#8377;{item?.price}
            </Typography>
            {item?.strikePrice && Number(item.strikePrice) > Number(item.price) && (
              <Typography sx={{ fontSize: "13px", color: "#71717A", textDecoration: "line-through" }}>
                &#8377;{item.strikePrice}
              </Typography>
            )}
          </Box>
          <Typography sx={{ fontSize: "11px", color: "#52525B", fontFamily: "Inter" }}>per person</Typography>
        </Box>
        <Button
