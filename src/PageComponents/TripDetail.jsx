...
const TD_CSS = `
  .td-page { font-family: ${BODY}; background: var(--surface-soft, #EFEAE1); min-height: 100vh; text-align: left; }
  ...
  .td-cta:hover { transform: translateY(-2px); box-shadow: 0 14px 30px rgba(205,72,42,.32); background: #A63A22; }
  .td-ghost { transition: background .16s ease, border-color .16s ease; }
  .td-ghost:hover { background: #EFEAE1; border-color: ${ACCENT}; }
...
export default function TripDetail({ initialRaw, initialReviews }) {
  ...
  if (tripsRes && !raw) return <div style={{ minHeight: "60vh", display: "grid", placeItems: "center", color: "#6B6355" }}>Trip not found.</div>;
  if (!trip) return <div style={{ minHeight: "60vh", display: "grid", placeItems: "center", color: "#6B6355" }}>Loading…</div>;
  ...
  const sectionH2 = { margin: "0 0 16px", fontFamily: DISPLAY, fontWeight: 700, fontSize: "clamp(22px,2.6vw,27px)", letterSpacing: "-.01em", color: "#1E1C18" };
  const card = { background: "#FFFFFF", border: "1px solid #E1DACE", borderRadius: "16px" };
  ...
  return (
    <div className="td-page" ref={rootRef}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "clamp(18px,2.5vw,32px) clamp(16px,4vw,48px) 0" }}>
        ...
        <div className="td-grid">
          ...
          <div style={{ minWidth: 0 }}>
            <h1 style={{ margin: 0, fontFamily: DISPLAY, fontWeight: 700, fontSize: "clamp(28px,3.8vw,42px)", lineHeight: 1.06, letterSpacing: "-.02em", color: "#1E1C18", textWrap: "balance" }}>{trip.title}</h1>
            <div style={{ display: "flex", alignItems: "center", gap: "20px", flexWrap: "wrap", marginTop: "14px" }}>
              {trip.location && <span style={{ display: "flex", alignItems: "center", gap: "6px", font: `500 14.5px/1 ${BODY}`, color: "#6B6355" }}><span style={{ color: ACCENT, display: "flex" }}><Ic w={15} paths={["M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z", "M12 10a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"]} /></span>{trip.location}</span>}
              {trip.duration && <span style={{ display: "flex", alignItems: "center", gap: "6px", font: `500 14.5px/1 ${BODY}`, color: "#6B6355" }}><span style={{ color: ACCENT, display: "flex" }}><Ic w={15} paths={["M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Z", "M12 7v5l3 2"]} /></span>{trip.duration}</span>}
              {trip.hasReviews ? (
                <span style={{ display: "flex", alignItems: "center", gap: "6px", font: `600 14.5px/1 ${BODY}`, color: "#1E1C18" }}><span style={{ color: "#E0922F", display: "flex" }}><Ic w={15} fill="#E0922F" paths={["m12 2 3 6.5 7 .6-5.3 4.6 1.6 6.8L12 17l-6.2 3.5 1.6-6.8L2 9.1l7-.6L12 2Z"]} /></span>{trip.rating} <span style={{ color: "#857C6C", fontWeight: 500 }}>({trip.reviewCount} reviews)</span></span>
              ) : (
                <span style={{ display: "flex", alignItems: "center", gap: "6px", font: `600 13.5px/1 ${BODY}`, color: "#857C6C" }}>New experience</span>
              )}
            </div>

            {trip.categories.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "18px" }}>
                {trip.categories.map((c, i) => <span key={i} style={{ padding: "7px 14px", borderRadius: "99px", background: "var(--brand-100, #F4D8CE)", border: "1px solid #E1DACE", color: ACCENT, font: `600 13px/1 ${BODY}` }}>{c.label}</span>)}
              </div>
            )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: "12px", marginTop: "24px" }}>
                {trip.facts.map((f) => (
                  <div key={f.key} style={{ ...card, padding: "16px 18px" }}>
                    ...
                    <div style={{ font: `400 12px/1 ${BODY}`, color: "#857C6C" }}>{f.label}</div>
                    <div style={{ marginTop: "4px", font: `700 15px/1.25 ${BODY}`, color: "#1E1C18" }}>{f.value}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* STICKY TABS */}
            <div style={{ position: "sticky", top: 64, zIndex: 20, background: "var(--surface-soft, #EFEAE1)", paddingTop: "16px", marginTop: "30px" }}>
              <div style={{ display: "flex", gap: "28px", borderBottom: "1px solid #E1DACE", overflowX: "auto" }}>
                {TABS.map(([id, label]) => (
                  <button key={id} type="button" className="td-tab" onClick={goTo(id, label)} style={{ padding: "14px 2px", font: `600 14px/1 ${BODY}`, color: active === label ? ACCENT : "#857C6C", borderBottom: `2px solid ${active === label ? ACCENT : "transparent"}`, whiteSpace: "nowrap" }}>{label}</button>
                ))}
              </div>
            </div>
            ...
            <section id="td-overview" className="td-sec" style={{ marginTop: "34px" }}>
              ...
              <div style={{ ...card, padding: "clamp(20px,2.4vw,28px)" }}>
                <p style={{ margin: 0, font: `400 16px/1.75 ${BODY}`, color: "#33302A", whiteSpace: "pre-line" }}>{trip.overview || "Details coming soon."}</p>
                {trip.highlights.length > 0 && (
                  <>
                    <div style={{ font: `700 12px/1 ${BODY}`, letterSpacing: ".08em", textTransform: "uppercase", color: "#A79D8C", margin: "24px 0 14px", paddingTop: "20px", borderTop: "1px solid #E1DACE" }}>Highlights</div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: "12px" }}>
                      {trip.highlights.map((h, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "11px", font: `400 15px/1.55 ${BODY}`, color: "#33302A" }}>
            ...
            {/* HOST */}
            {trip.host && (
              <section id="td-host" className="td-sec" style={{ marginTop: "40px", background: "#1E1C18", borderRadius: "22px", padding: "clamp(24px,3vw,34px)", color: "#F7F4EF", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", alignItems: "flex-start" }}>
                    <div style={{ position: "relative", flex: "none" }}>
                      <div style={{ width: 80, height: 80, borderRadius: "18px", background: `linear-gradient(150deg,#D9714F,${ACCENT})`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontWeight: 700, fontSize: "33px", color: "#FBF0EC" }}>{trip.host.initial}</div>
                      {trip.host.verified && <span style={{ position: "absolute", right: -8, bottom: -8, width: 30, height: 30, borderRadius: "50%", background: "#2F5D50", border: "3px solid #1E1C18", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}><Ic w={15} paths={["M20 6 9 17l-5-5"]} /></span>}
                    </div>
                    <div style={{ flex: 1, minWidth: 240 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                        <span style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: "24px", letterSpacing: "-.01em", color: "#F7F4EF" }}>{trip.host.name}</span>
                      ...
                      <div style={{ display: "flex", gap: "18px", flexWrap: "wrap", marginTop: "10px" }}>
                        <span style={{ font: `600 13px/1 ${BODY}`, color: "#E1DACE" }}>{trip.host.tripsHosted} <span style={{ color: "#9C9388", fontWeight: 400 }}>trips hosted</span></span>
                        {trip.host.years && <span style={{ font: `600 13px/1 ${BODY}`, color: "#E1DACE" }}>{trip.host.years} <span style={{ color: "#9C9388", fontWeight: 400 }}>hosting</span></span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "24px", marginTop: "20px" }}>
                    {trip.host.languages.length > 0 && (
                      <div>
                        <div style={{ font: `700 11px/1 ${BODY}`, letterSpacing: ".08em", textTransform: "uppercase", color: "#857C6C", marginBottom: "8px" }}>Speaks</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>{trip.host.languages.map((l, i) => <span key={i} style={{ padding: "5px 11px", borderRadius: "99px", border: "1px solid rgba(244,238,228,.22)", font: `600 12px/1 ${BODY}`, color: "#E1DACE" }}>{l.label}</span>)}</div>
                      </div>
                    )}
                    {trip.host.regions.length > 0 && (
                      <div>
                        <div style={{ font: `700 11px/1 ${BODY}`, letterSpacing: ".08em", textTransform: "uppercase", color: "#857C6C", marginBottom: "8px" }}>Regions</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>{trip.host.regions.map((r, i) => <span key={i} style={{ padding: "5px 11px", borderRadius: "99px", border: "1px solid rgba(244,238,228,.22)", font: `600 12px/1 ${BODY}`, color: "#E1DACE" }}>{r.label}</span>)}</div>
                      </div>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginTop: "24px" }}>
                    ...
                    <button type="button" onClick={goHost} style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "12px 20px", font: `700 14px/1 ${BODY}`, color: "#F7F4EF", background: "transparent", border: "1px solid rgba(244,238,228,.3)", borderRadius: "11px", cursor: "pointer" }}>Message host</button>
                  </div>
                </div>
              </section>
            )}
            ...
            {itinerary.length > 0 && (
              <section id="td-itinerary" className="td-sec" style={{ marginTop: "40px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  {itinerary.map((d, i) => (
                    <div key={i} style={{ display: "flex", gap: "16px", background: "#FFFFFF", border: "1px solid #E1DACE", borderRadius: "16px", padding: "20px 22px", boxShadow: "0 1px 2px rgba(60,42,28,.04)" }}>
                      ...
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ font: `700 10.5px/1 ${BODY}`, letterSpacing: ".1em", textTransform: "uppercase", color: "#A79D8C", marginBottom: "5px" }}>Day {Number(d.num)}</div>
                        <div style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: "18px", lineHeight: 1.25, color: "#1E1C18" }}>{d.title}</div>
                        {d.desc && <p style={{ margin: "8px 0 0", font: `400 14.5px/1.7 ${BODY}`, color: "#4E483D", whiteSpace: "pre-line" }}>{d.desc}</p>}
                        {d.tags.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "12px" }}>{d.tags.map((t, j) => <span key={j} style={{ padding: "4px 10px", borderRadius: "99px", background: "var(--brand-100, #F4D8CE)", border: "1px solid #E1DACE", font: `600 11px/1 ${BODY}`, color: ACCENT }}>{typeof t === "string" ? t : t.label}</span>)}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                {itineraryMore && (
                  <button type="button" onClick={() => setItineraryOpen((v) => !v)} className="td-ghost" style={{ marginTop: "6px", display: "inline-flex", alignItems: "center", gap: "8px", padding: "11px 20px", border: "1px solid #E1DACE", background: "#FFFFFF", borderRadius: "11px", font: `600 13.5px/1 ${BODY}`, color: "#4E483D", cursor: "pointer" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "16px" }}>
                  {trip.inclusions.length > 0 && (
                    <div style={{ ...card, padding: "22px 24px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "9px", marginBottom: "16px" }}><span style={{ color: "#2E6E4E" }}><Ic w={19} paths={["M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20Z", "m8 12 3 3 5-6"]} /></span><span style={{ font: `700 15px/1 ${BODY}`, color: "#1E1C18" }}>Included</span></div>
                      <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "12px" }}>
                        {trip.inclusions.map((it, i) => <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "10px", font: `400 14.5px/1.55 ${BODY}`, color: "#4E483D" }}><span style={{ flex: "none", color: "#2E6E4E", marginTop: 2 }}><Ic w={14} paths={["M20 6 9 17l-5-5"]} /></span><span>{it.label}</span></li>)}
                      </ul>
                    </div>
                  )}
                  {trip.exclusions.length > 0 && (
                    <div style={{ ...card, padding: "22px 24px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "9px", marginBottom: "16px" }}><span style={{ color: "#B23A2E" }}><Ic w={19} paths={["M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20Z", "m15 9-6 6M9 9l6 6"]} /></span><span style={{ font: `700 15px/1 ${BODY}`, color: "#1E1C18" }}>Not included</span></div>
                      <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "12px" }}>
                        {trip.exclusions.map((e, i) => <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "10px", font: `400 14.5px/1.55 ${BODY}`, color: "#4E483D" }}><span style={{ flex: "none", color: "#B23A2E", marginTop: 2 }}><Ic w={14} paths={["M18 6 6 18M6 6l12 12"]} /></span><span>{e.label}</span></li>)}
                      </ul>
                    </div>
                  )}
                </div>
            ...
            <section id="td-reviews" className="td-sec" style={{ marginTop: "40px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                ...
                <span style={{ padding: "4px 11px", borderRadius: "99px", background: "var(--brand-100, #F4D8CE)", border: "1px solid #E1DACE", color: ACCENT, font: `600 11px/1 ${BODY}` }}>This experience only</span>
              </div>
              {!trip.hasReviews ? (
                <div style={{ marginTop: "14px", textAlign: "center", padding: "44px 24px", background: "#FFFFFF", border: "1px dashed #E1DACE", borderRadius: "16px" }}>
                  <p style={{ margin: "0 0 6px", font: `700 16px/1.4 ${BODY}`, color: "#1E1C18" }}>No reviews yet</p>
                  <p style={{ margin: "0 auto", maxWidth: 420, font: `400 14px/1.55 ${BODY}`, color: "#857C6C" }}>This is a new experience. Be one of the first to travel {trip.host ? `with ${trip.host.name}` : "with this host"} and share your story.</p>
                </div>
              ) : (
                <>
                  <p style={{ margin: "0 0 18px", font: `400 13.5px/1.4 ${BODY}`, color: "#857C6C" }}>From travellers who booked this trip — separate from host and brand reviews.</p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "26px", alignItems: "center", padding: "22px 26px", background: "var(--brand-100, #F4D8CE)", border: "1px solid #E1DACE", borderRadius: "16px", marginBottom: "20px" }}>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: "46px", lineHeight: 1, color: "#1E1C18" }}>{trip.rating}</div>
                      <div style={{ marginTop: "5px", color: "#E0922F", fontSize: "14px", letterSpacing: "2px" }}>★★★★★</div>
                      <div style={{ marginTop: "4px", font: `500 12px/1 ${BODY}`, color: "#857C6C" }}>{trip.reviewCount} reviews</div>
                    </div>
                    <div style={{ flex: 1, minWidth: 200, display: "flex", flexDirection: "column", gap: "7px" }}>
                      {trip.ratingBars.map((bar) => (
                        <div key={bar.stars} style={{ display: "flex", alignItems: "center", gap: "10px" }}><span style={{ font: `600 12px/1 ${BODY}`, color: "#857C6C", width: 28 }}>{bar.stars}★</span><span style={{ flex: 1, height: 7, borderRadius: "99px", background: "#F3E8E2", overflow: "hidden" }}><span style={{ display: "block", height: "100%", width: bar.pct, background: ACCENT }} /></span></div>
                      ))}
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: "14px" }}>
                    {trip.reviews.map((rv, i) => (
                      <div key={i} style={{ padding: "18px 20px", border: "1px solid #E1DACE", borderRadius: "14px", background: "#FFFFFF" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "11px" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}><div style={{ font: `600 14px/1.2 ${BODY}`, color: "#1E1C18" }}>{rv.name}</div>{rv.date && <div style={{ font: `400 11.5px/1 ${BODY}`, color: "#857C6C", marginTop: 2 }}>{rv.date}</div>}</div>
                          <span style={{ color: "#E0922F", fontSize: "12px" }}>{rv.stars}</span>
                        </div>
                        <p style={{ margin: "12px 0 0", font: `400 14px/1.65 ${BODY}`, color: "#4E483D" }}>{rv.text}</p>
                      </div>
                    ))}
                  </div>
                </>
              ...
              <section id="td-info" className="td-sec" style={{ marginTop: "40px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {trip.infoGroups.map((g, i) => (
                    <details key={i} className="td-acc" open={!!g.open} style={{ border: "1px solid #E1DACE", borderRadius: "14px", background: "#FFFFFF", overflow: "hidden" }}>
                      <summary style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", padding: "17px 20px", fontFamily: DISPLAY, fontWeight: 700, fontSize: "16px", color: "#1E1C18" }}>{g.title}<span className="td-acc-ic" style={{ flex: "none", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: "#FCE8E1", color: ACCENT, transition: "transform .2s ease", fontSize: "16px" }}>+</span></summary>
                      <div style={{ padding: "0 20px 18px" }}>
                        {g.kind === "list" && (
                          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
                            {g.items.map((it, j) => <li key={j} style={{ display: "flex", alignItems: "flex-start", gap: "10px", font: `400 14.5px/1.6 ${BODY}`, color: "#4E483D" }}><span style={{ flex: "none", width: 6, height: 6, marginTop: 8, borderRadius: "50%", background: ACCENT }} /><span>{typeof it === "string" ? it : it.label}</span></li>)}
                          </ul>
                        )}
                        {g.kind === "text" && <p style={{ margin: 0, font: `400 14.5px/1.7 ${BODY}`, color: "#4E483D", whiteSpace: "pre-line", maxWidth: "70ch" }}>{g.text}</p>}
                        {g.kind === "faq" && (
                          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                            {g.faqs.map((q, j) => <div key={j}><div style={{ font: `600 14.5px/1.4 ${BODY}`, color: "#1E1C18" }}>{q.q}</div><p style={{ margin: "5px 0 0", font: `400 14px/1.6 ${BODY}`, color: "#6B6355", maxWidth: "70ch" }}>{q.a}</p></div>)}
                          </div>
                        )}
                      </div>
                    </details>
          ...
          <aside className="td-side" style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ background: "#FFFFFF", border: "1px solid #E1DACE", borderRadius: "18px", overflow: "hidden", boxShadow: "0 12px 30px -16px rgba(0,0,0,.15)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "9px", padding: "12px 20px", background: "var(--brand-100, #F4D8CE)", borderBottom: "1px solid #E1DACE" }}>
              ...
              <div style={{ padding: "18px 22px 20px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "5px" }}>
                  <span style={{ font: `600 11px/1 ${BODY}`, color: "#857C6C" }}>Starting from</span>
                  {trip.hasOff && <span style={{ padding: "4px 10px", borderRadius: "99px", background: "#E0EFE4", color: "#2E6E4E", font: `700 10px/1 ${BODY}` }}>✦ {trip.off}% OFF</span>}
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "9px", flexWrap: "wrap", marginBottom: "16px" }}>
                  <span style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: "32px", letterSpacing: "-.02em", color: "#1E1C18" }}>₹ {trip.price}</span>
                  {trip.hasStrike && <span style={{ font: `600 13px/1 ${BODY}`, color: "#B3ABA3", textDecoration: "line-through" }}>₹ {trip.strikePrice}</span>}
                  <span style={{ font: `500 12px/1 ${BODY}`, color: "#857C6C" }}>/ person</span>
                </div>
                ...
                {trip.partialEnabled && (
                  <p style={{ margin: "10px 0 0", textAlign: "center", font: `600 11.5px/1.4 ${BODY}`, color: "#2E6E4E" }}>
                    Book now for ₹ {inr(trip.firstBookingPrice)} — balance due 15 days before departure
                  </p>
                )}
                <p style={{ margin: "10px 0 0", textAlign: "center", font: `400 11.5px/1.4 ${BODY}`, color: "#857C6C" }}>Select a batch date on the next step</p>
              </div>
              <div style={{ padding: "14px 22px", borderTop: "1px solid #E1DACE", background: "var(--brand-100, #F4D8CE)", display: "flex", flexDirection: "column", gap: "10px" }}>
                {["Secure payments · verified host", "Hand-picked, community-first trip", "On-platform host support"].map((t, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", font: `400 12.5px/1.3 ${BODY}`, color: "#4E483D" }}><span style={{ color: "#2E6E4E", flex: "none", display: "flex" }}><Ic w={15} paths={["M20 6 9 17l-5-5"]} /></span>{t}</div>
                ))}
              </div>
            </div>

            {trip.host && (
              <div style={{ background: "#FFFFFF", border: "1px solid #E1DACE", borderRadius: "18px", padding: "18px 22px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "11px" }}>
                  <span style={{ width: 42, height: 42, flex: "none", borderRadius: "11px", background: `linear-gradient(150deg,#D9714F,${ACCENT})`, color: "#FBF0EC", display: "flex", alignItems: "center", justifyContent: "center", font: `700 16px/1 ${BODY}` }}>{trip.host.initial}</span>
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ font: `400 11px/1 ${BODY}`, color: "#857C6C" }}>Your host</div><div style={{ marginTop: 3, font: `600 14px/1.2 ${BODY}`, color: "#1E1C18" }}>{trip.host.name}</div></div>
                </div>
                <button type="button" className="td-ghost" onClick={goHost} style={{ width: "100%", marginTop: "14px", padding: "12px", font: `700 13px/1 ${BODY}`, color: "#1E1C18", background: "#fff", border: "1px solid #E1DACE", borderRadius: "11px", cursor: "pointer" }}>Message on Nomadic Townies</button>
              </div>
            )}
          </aside>
        </div>
      ...
      {/* MOBILE STICKY CTA */}
      <div className="td-mcta" style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 40, display: "flex", alignItems: "center", gap: "12px", padding: "12px 16px", background: "#FFFFFF", borderTop: "1px solid #E1DACE" }}>
        <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: "19px", color: "#1E1C18" }}>₹ {trip.price}</div><div style={{ font: `400 11px/1 ${BODY}`, color: "#857C6C" }}>per person</div></div>
